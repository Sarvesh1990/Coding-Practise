...
void Klargest (Node * node, int k, int * count)
{
    if(*count<=k)
    {
        if(node->right!=NULL)
        {
            Klargest(node->right, k, count);
        }
        cout<<"Count is "<<*count<<endl;
        if(*count==k)
        {   
            cout<<k<<"th largest is "<<node->value<<endl;
            return;
        }
        *count=*count+1;
        if(node->left!=NULL)
        {
            cout<<"Left is there with count "<<*count<<endl;
            Klargest(node->left, k, count);
        }
    }
};
